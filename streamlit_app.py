...
st.title("PDF to Word Translator (2025/05/17/gemini-2.5-pro-preview-05-06)")
...
def ocr_image(image_bytes, client, ocr_prompt):
    # 送信前のデバッグ出力
    st.write(f">>> OCR request: image_bytes={len(image_bytes)} bytes, prompt_len={len(ocr_prompt)}")
    ...
    for attempt in range(2):
        try:
            ...
            # 受信後のデバッグ出力
            st.write(f">>> OCR response (attempt {attempt}): {res}")
            st.write(f">>>  candidates: {getattr(res, 'candidates', None)}")
            st.write(f">>>  text attr: {getattr(res, 'text', None)}")
            for i, cand in enumerate(res.candidates or []):
                st.write(f"  candidate[{i}].content.parts = {cand.content.parts}")

            return res.text.strip()
        except ClientError as e:
            # エラー時のデバッグ出力
            st.write(f"!!! OCR ClientError (attempt {attempt}): {e}")
